...
namespace TernaryVectorClass {
	...
	TernaryVector::TernaryVector(const TernaryVector& copy) noexcept : SZ(copy.SZ) {
		vector = new char[SZ];
		for (int i = 0; i < SZ; ++i)
	...
	TernaryVector& TernaryVector::operator = (const TernaryVector& copy) noexcept {
		if (this != &copy) {
			SZ = copy.SZ;
			char* old = vector;
			vector = new char[SZ];
	...
	TernaryVector TernaryVector::Invertion() const noexcept {
	...
	// Инвертирование вектора
	TernaryVector TernaryVector::operator ~() const noexcept {
		return Invertion();
	}


	// Выполнение порязрядной конъюнкции данного вектора с другим (Унарная операция)
	TernaryVector& TernaryVector::Conjunction(const TernaryVector& con) noexcept {
		int corrent_size, res_size;
		if (SZ <= con.SZ)
			corrent_size = SZ;
		else
			corrent_size = con.SZ;
	...
	TernaryVector& TernaryVector::operator&=(const TernaryVector& vector) noexcept {
	...
	const TernaryVector Disjunction(const TernaryVector& left, const TernaryVector& right) noexcept {
	...
	// Выполнение порязрядной дизъюнкции данного вектора с другим (бинарная операция)
	const TernaryVector operator | (const TernaryVector& vector_1, const TernaryVector& vector_2) noexcept {
	...
	const int Compare(const TernaryVector& left, const TernaryVector& right) noexcept {
		if (left.SZ != right.SZ)
	...
	const bool operator == (const TernaryVector& left, const TernaryVector& right) noexcept {
	...
	std::istream& operator >> (std::istream& stream, TernaryVector& vector) noexcept {
		std::string str;
		stream >> str;
	...
	std::ostream& operator << (std::ostream& stream, const TernaryVector& vector) noexcept {
		if (vector.SZ > 0) {
